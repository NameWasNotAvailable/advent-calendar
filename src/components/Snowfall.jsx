...
const Snowfall = ({ konamiCodeActivated }) => {
  useEffect(() => {
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      const snowflakeMaterial = new THREE.MeshBasicMaterial({
        color: konamiCodeActivated ? randomColor : 0xffffff,
      })
    ...
    const moonTexture = new THREE.TextureLoader().load(
      konamiCodeActivated ? '/assets/train.png' : '/assets/moonTexture.png',
    )
    ...
    const forestMaterial = new THREE.MeshBasicMaterial({
      color: konamiCodeActivated ? 0x83f52c : 0x1f1830,
    })
  ...
  return (
    <div
      id="snow-container"
      style={{
        background: konamiCodeActivated
          ? 'linear-gradient(#f409ff, #f4ff00)'
          : 'linear-gradient(#16092e, #7E87DE)',
      }}
    />
  )
}
